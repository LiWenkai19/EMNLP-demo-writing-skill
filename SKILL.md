...
description: 写作或修改 EMNLP/ACL/NAACL System Demonstrations（demo track）投稿论文时使用。覆盖从论文骨架、各章节模板、引言写法、evaluation 设计、论文图表绘制（系统架构图/流程图/数据图表/界面截图，含 TikZ/Mermaid/Graphviz/matplotlib 模板）、ACL 官方 LaTeX 排版规范与可套用的 .tex 骨架、到官方 8 个核心问题自检与投稿前 desk-reject 规避清单的全流程。当用户要写 demo 论文、起草某一章节、画论文里的图/架构图/Figure、搭 LaTeX 骨架或问排版/引用/页数格式、检查 demo 论文是否合规、或问“demo 论文怎么写/结构是什么”时触发。
---
...
- **“LaTeX 怎么排 / 用什么模板 / 帮我搭论文骨架 / 引用怎么写 / 页数格式”** → 走【LaTeX 排版】，见 `templates/latex-formatting.md`，给官方模板用法与可直接套用的 .tex 骨架。
...
EMNLP/ACL/NAACL 统一用 ACL 官方模板（`acl.sty`），详见 `templates/latex-formatting.md`，内含可直接复制的 demo 论文 .tex 骨架。要点：

- 投稿用 `\usepackage[review]{acl}`（带行号），录用后改 `\usepackage{acl}` 出 camera-ready。
- 字体 Times Roman 11pt；引用用 `\citep{}` / `\citet{}`；参考文献可引入 `anthology.bib` 全库。
- 正文 ≤ 6 页；References / Limitations / Ethics Statement **不计页数**；附录 ≤ 2 页。
- 含中文示例（如中医术语）用 XeLaTeX + `ctex`。
- 健康/医疗类 demo **务必写 Ethics Statement**（声明非诊断、数据隐私）。
- 官方模板：https://github.com/acl-org/acl-style-files
...
- ACL 官方 LaTeX 排版规范 + 论文骨架：`templates/latex-formatting.md`
